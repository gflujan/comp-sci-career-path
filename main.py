...
def start_game():
  ...
  money_response = input(
    f'Hi, {player_name}! Are you ready to win some money?! ([y]es or [n]o): ')
  print()
  ...
  print(
    "DISCLAIMER: This isn't ACTUAL money, it's just pretend. I don't owe you anything. ;-D\n"
  )

  while num_strikes < 3:
    if current_level == 1 and is_retry != True:
      knows_how_to_play = input(
        'Do you already know how to play? ([y]es or [n]o): ')
      print()

      if knows_how_to_play.lower() == 'yes' or knows_how_to_play.lower(
      ) == 'y':
    ...
    if result == 'walk':
      ...
      print(
        f'You have {num_strikes} {ns_display}. But we are going to try again. :-)'
      )
      is_retry = True
